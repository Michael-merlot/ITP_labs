...
// current инициализируется первым символом входной строки.
        for (int i = 1; i < str.length(); i++) { // Цикл, который идёт с 1-го символа до последнего в строке.
            if (Math.abs(str.charAt(i) - str.charAt(i - 1)) == 1) { // Проверка, является ли текущий символ соседним по алфавиту с предыдущим символом.
...
// Брать цифру можно только один раз.
    ...
    public static int switchNums(int num1, int num2) {
        ...
        for (int index2 = 0; index2 < digits2.length; index2++) { // индекса для первого массива символов, начиная с последнего элемента.
