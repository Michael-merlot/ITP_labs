...
public class HashTable<K, V> { // типы ключа
    private class Entry { // вложенный класс для хранения пар Ключ-значение
        K key; // имеет конструктор для иниц и методы для доступа к полям
        V value;

        public Entry(K key, V value) { // конструктор
            this.key = key; // присваивает значени аргумента key -> key
            this.value = value;
        }

        public K getKey() { // метод для получения значения ключа
            return key;
        }

        public V getValue() { // метод для получения значения
            return value;
        }
...
// Поля и конструкторы основного класса
